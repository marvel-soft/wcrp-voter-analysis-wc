#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# wcrp-precinct-voter-stats-wc
# this is for washow county only
#
#
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#use strict;
...
my $inputFile = "../prod-in1/2019 2nd Free List 3.19.19.csv";
#my $inputFile = "../test-in/2019 1st Free List 5pre.csv";
#my $inputFile = "../test-in/2019 1st Free List 1.7.19-250-low.csv";
...
sub main {
	...
	$voterFile = "precinct-voters-" . $fileName . ".csv";
	printLine ("Voter Profile file: $voterFile \n");
	open( $voterFileh, ">$voterFile" )
	  or die "Unable to open $voterFileh: $voterFile Reason: $!";
	print $voterFileh $voterHeading;

	$voterStatFile = "precinct-voterstat-" . $fileName . ".csv";
	printLine ("Voter Profile file: $voterStatFile \n");
	...
	open( $precinctFileh, ">$precinctFile" )
	  or die "Unable to open $voterFileh: $precinctFile Reason: $!";
  ...
  NEW:
	while ( $line1Read = <INPUT> ) {
		...
		my $now            = localtime;
		my $daysLastRegistered = $now - $before;
		$daysLastRegistered = ( $daysLastRegistered / (86400) );
		$daysLastRegistered = round($daysLastRegistered);
		#debug
		if ($daysLastRegistered < 0 ) {
			print $daysLastRegistered;
		}
		$newLine{"Days Last Reg"} = int($daysLastRegistered);
		...
		$before = Time::Piece->strptime( $adjustedDate, "%m/%d/%Y" );
		$now            = localtime;
		$daysTotlRegistered = $now - $before;
		$daysTotlRegistered = ( $daysTotlRegistered / (86400) );
		$daysTotlRegistered = round($daysTotlRegistered);
		#debug
		if ($daysTotlRegistered < 0 ) {
			print "$daysTotlRegistered . $voter_id";
		}
		...
		print $voterFileh join( ',', @voterProfile ), "\n";
		...
		$statLine{"Precinct"}        = substr $csvRowHash{"precinct"}, 0, 6;
		$statLine{"Voter Status"}    = $csvRowHash{"status"};
		$statLine{"Last Name"}       = $csvRowHash{"name_last"};
		$statLine{"gender"}          = $csvRowHash{"gender"};
		$statLine{"military"}        = $csvRowHash{"military"};
		$statLine{"Reg_DateOrig"}    = "$mm/$dd/$yy";
		$statLine{"Days Totl Reg"}   = int($daysTotlRegistered);

		#birthdate
		$statLine{"Age"}       = int($age);
...
printLine ("<===> Total Precincts written: $precinctsWritten \n");
